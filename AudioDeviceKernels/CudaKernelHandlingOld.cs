...
namespace GenericCudaCLI
{
	public class CudaKernelHandlingOld
	{
		// ----- ATTRIBUTES ----- \\
		...
		// ----- CONSTRUCTOR ----- \\
		public CudaKernelHandlingOld(CudaHandling cudaHandling, PrimaryContext ctx)
		{
		...
		public void Log(string message, string inner = "", int level = 1, bool update = false)
		{
			...
			if (update)
			{
				// Overwrite last Output Line with new message
